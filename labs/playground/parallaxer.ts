...
export class Parallaxer {
  ...
  render_() {
    ...
    const xDegree =
      this.mousePosition_.percentageX * this.rotationSensitivity.x;
    const yDegree =
      this.mousePosition_.percentageY * this.rotationSensitivity.y;

    const xTrans =
      -this.mousePosition_.percentageX * this.translateSensitivity.x;
    const yTrans =
      -this.mousePosition_.percentageY * this.translateSensitivity.y;
    ...
    this.transformValues.xTrans +=
      (xTrans - this.transformValues.xTrans) * 0.03;
    this.transformValues.yTrans +=
      (yTrans - this.transformValues.yTrans) * 0.03;

    const transformString =
      'rotateX(' +
      this.transformValues.yDeg +
      'deg) rotateY(' +
      -this.transformValues.xDeg +
      'deg) rotateZ(0deg)' +
      ' translate(' +
      -this.transformValues.xTrans +
      'px, ' +
