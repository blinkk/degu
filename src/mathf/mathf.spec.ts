...
test('childProgress', t => {
  ...
  t.is(mathf.roundToPrecision(mathf.childProgress(0.3, 0.2, 0.6), 2), 0.25);
  t.is(mathf.roundToPrecision(mathf.childProgress(0.3, 0.2, 0.6), 2), 0.25);
  t.is(mathf.roundToPrecision(mathf.childProgress(0.4, 0.2, 0.6), 2), 0.5);
  t.is(mathf.roundToPrecision(mathf.childProgress(0.5, 0.2, 0.6), 2), 0.75);
  t.is(mathf.roundToPrecision(mathf.childProgress(0.6, 0.2, 0.6), 2), 1);
  t.is(mathf.roundToPrecision(mathf.childProgress(0.7, 0.2, 0.6), 2), 1);
});
...
test('interpolateRange', t => {
  t.is(mathf.interpolateRange(0, 0, 100, 0, 200), 0);
  t.is(mathf.interpolateRange(50, 0, 100, 0, 200), 100);
  t.is(mathf.interpolateRange(30, 0, 100, 0, 200), 60);
  t.is(mathf.int(mathf.interpolateRange(30, 10, 120, 800, 1000)), 855);
});
...
test('angleDistanceDegree', t => {
  ...
  t.is(mathf.int(mathf.angleDistanceDegree(30, 10)), -20);
  t.is(mathf.int(mathf.angleDistanceDegree(10, 50)), 40);
  t.is(mathf.int(mathf.angleDistanceDegree(180, 10)), -170);
  t.is(mathf.int(mathf.angleDistanceDegree(10, 340)), -30);
});

test('scaleX', t => {
  t.is(mathf.scaleX(5, 10, 20), 10);
  t.is(mathf.scaleX(2, 10, 20), 4);
});

test('scaleY', t => {
  t.is(mathf.scaleY(5, 10, 3), 6);
  t.is(mathf.scaleY(2, 10, 20), 100);
});
...
test('resizedimensionalBoxToWidth', t => {
  ...
  t.deepEqual(mathf.resizeDimensionalBoxToWidth({width: 10, height: 12}, 800), {
    width: 800,
    height: 960,
  });
});
...
test('collisionPointVersusConvexPolygon', t => {
  ...
  t.is(mathf.collisionPointVersusConvexPolygon({x: 0, y: 1}, poly), true);
  t.is(mathf.collisionPointVersusConvexPolygon({x: -1, y: 1}, poly), false);
  t.is(mathf.collisionPointVersusConvexPolygon({x: 2, y: 1}, poly), true);
  t.is(mathf.collisionPointVersusConvexPolygon({x: 4, y: 1}, poly), false);
  t.is(mathf.collisionPointVersusConvexPolygon({x: 2, y: 2}, poly), true);
});

test('getValueInRangeByProgress', t => {
  // Simple test.
  t.is(mathf.getValueInRangeByProgress(-10, 0, 10), 0);
  t.is(mathf.getValueInRangeByProgress(0, 0, 10), 0);
  t.is(mathf.getValueInRangeByProgress(0.2, 0, 10), 2);
  t.is(mathf.getValueInRangeByProgress(0.5, 0, 10), 5);
  t.is(mathf.getValueInRangeByProgress(1, 0, 10), 10);
  t.is(mathf.getValueInRangeByProgress(10, 0, 10), 10);

  // Shift
  t.is(mathf.getValueInRangeByProgress(0, 2, 12), 2);
  t.is(mathf.getValueInRangeByProgress(0.2, 2, 12), 4);
  t.is(mathf.getValueInRangeByProgress(0.5, 2, 12), 7);
  t.is(mathf.getValueInRangeByProgress(1, 2, 12), 12);
});

test('getProgressInRangeByValue', t => {
  // Simple test.
  t.is(mathf.getProgressInRangeByValue(0, 0, 10), 0);
  // Out of range
  t.is(mathf.getProgressInRangeByValue(-10, 0, 10), 0);
  t.is(mathf.getProgressInRangeByValue(2, 0, 10), 0.2);
  t.is(mathf.getProgressInRangeByValue(5, 0, 10), 0.5);
  t.is(mathf.getProgressInRangeByValue(10, 0, 10), 1);
  // Out of range
  t.is(mathf.getProgressInRangeByValue(100, 0, 10), 1);

  // Shift
  t.is(mathf.getProgressInRangeByValue(2, 2, 12), 0);
  t.is(mathf.getProgressInRangeByValue(4, 2, 12), 0.2);
  t.is(mathf.getProgressInRangeByValue(7, 2, 12), 0.5);
  t.is(mathf.getProgressInRangeByValue(12, 2, 12), 1);
});
...
test('calculateBackgroundContain', t => {
  ...
  t.is(mathf.calculateBackgroundContain(parentBox, childBox), 10);
  t.is(mathf.calculateBackgroundContain(childBox, parentBox), 0.1);

  parentBox = {width: 100, height: 100};
  childBox = {width: 200, height: 100};
  t.is(mathf.calculateBackgroundContain(parentBox, childBox), 0.5);
});
...
test('calculateBackgroundCover', t => {
  ...
  let parentBox = {width: 500, height: 500};
  let childBox = {width: 50, height: 50};
  t.deepEqual(mathf.calculateBackgroundCover(parentBox, childBox), {
    width: 500,
    height: 500,
    xOffset: 0,
    yOffset: 0,
