...
export class MultiInterpolate {
   ...
   * @param config
   */
  private createStaggers(
    config: multiInterpolateConfig
  ): multiInterpolateConfig {
    ...
    this.config.interpolations.forEach((config: interpolateSettings) => {
      ...
      if (config.noInterpolation) {
        if (!config.noInterpolationDefault) {
          throw new Error(
            'You are using noInterpolation without a noInterpolation default'
          );
        }
        ...
        const interpolatedValue = new Interpolate({
          ...
          easeFunction: matchedRangeProgress.easingFunction || EASE.linear,
