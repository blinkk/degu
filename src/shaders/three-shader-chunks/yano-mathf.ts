 ...
 *
 * mat2 mat = yanoGetRotationMatrix(rotationInRadians);
 ...
 *
 */
const yanoGet2dRotationMatrix = `
   mat2 yanoGet2dRotationMatrix(float theta) {
    float s = sin(theta);
    float c = cos(theta);
    return mat2(c, -s, s, c);
   }
 ...
 *
 * mat2 scaleMat = yanoGetScaleMatrix(1.5);
 * mat2 rotateMat = yanoGetRotationMatrix(rotationInRadians);
 * vec2 scaledPoint = scaleMat * v_position.xy;
 * vec2 scaledAndRotatedPoint = scaleMat * rotateMat * v_position.xy;
 *
 */
const yanoGet2dScaleMatrix = `
  mat2 yanoGetScaleMatrix(float scale) {
 ...
 * // Rotate by 1 radian
 * vec2 rotatedPoints = yanoRotate2d(myPoints, 1);
 *
 */
const yanoRotate2d = `
   vec2 yanoRotate2d(vec2 pt, float theta) {
       mat2 rotationMatrix = yanoGet2dRotationMatrix(theta);
 ...
 * ```
 */
const yanoRotate2dImage = `
   vec3 yanoRotate2dImage(sampler2D texture, float aspect, vec2 uv, vec2 center, float theta, vec3 backgroundColor) {
...
export const yanoMathf = (three:any)=> {
  three.ShaderChunk.yanoMathf = `
  ${yanoGet2dRotationMatrix}
  ${yanoGet2dScaleMatrix}
  ${yanoRotate2d}
  ${yanoRotate2dImage}
  ${normalizedSin}
`;
